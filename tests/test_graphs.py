...
def test_plot_mic_distribution_all_branches(mock_plt):
    ...
    plot_mic_distribution(
        low, high, weights,
        dilution_factor=2,
        mus=[1], sigmas=[0.5],       # replaces mu1/sigma1
        pis=[1.0],                   # single component
    ...
    # -------- Case 2: Two-component mixture + custom proportions --------
    ax2 = MagicMock()
    ...
    plot_mic_distribution(
        low, high, weights,
        dilution_factor=2,
        mus=[1, 3],                  # replaces mu1/mu2
        sigmas=[0.4, 1.0],           # replaces sigma1/sigma2
        pis=[2, 1],                  # tests normalization
        ax=None
    )

    # Should plot comp1, comp2, mixture → ≥ 3 curves
    assert ax2.plot.call_count >= 3


    # -------- Case 3: Provided axis should be used --------
    provided_ax = MagicMock()
    out_ax = plot_mic_distribution(
        low, high, weights,
        dilution_factor=2,
        mus=[0], sigmas=[1], pis=[1],
        ax=provided_ax
    )
